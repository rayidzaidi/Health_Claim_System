...
export default function OfficerDashboard() {
  ...
  useEffect(() => {
    setMounted(true);
    const cachedStats = sessionStorage.getItem("cached_officer_stats");
    const cachedRecent = sessionStorage.getItem("cached_officer_recent_claims");
    const cachedAll = sessionStorage.getItem("cached_officer_all_claims");
    ...
    const fetchStats = async () => {
      try {
        ...
        setStats(res.data);
        sessionStorage.setItem("cached_officer_stats", JSON.stringify(res.data));
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
    ...
    const fetchClaims = async () => {
      try {
        ...
        sessionStorage.setItem("cached_officer_all_claims", JSON.stringify(res.data));
        sessionStorage.setItem("cached_officer_recent_claims", JSON.stringify(sliced));
      } catch (err) {
        console.error(err);
      }
    };
  ...
  if (loading && !stats) {
    return (
      <DashboardSkeleton
        title="Claims Officer Dashboard"
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Claims Officer Dashboard</h1>
